...
app.get("/search", async (c) => {
    ...
    const query = c.req.query("q"); // Query parameter that is sent from the client => http://.../api/search?q=coim
    ...
    const rank = await redis.zrank("districts", query.toUpperCase());
    ...
    if(rank !== null || rank !== undefined){
        ...
        const temp = await redis.zrange<string[]>( "districts", rank, rank!+100 ); // Returns a string array with all the possible combinations of the district name

        for(const el of temp){
            if(!el.endsWith(query)) break;
